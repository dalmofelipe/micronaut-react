...
import type { IBook, IBookRequest } from "../types/Book";
...
export const create = async (data: IBookRequest): Promise<IBook> => {
  const response = await api.post('/books', data);
  return response.data;
};

export const update = async (id: number, data: IBookRequest): Promise<IBook> => {
